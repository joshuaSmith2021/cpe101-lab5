...
import unittest
from filter import are_even, remove_duplicates, are_divisible_by_n
...
class TestPoly(unittest.TestCase):
    ...
    def test_are_even(self):
        ...
        self.assertListEqual(are_even([4, 7, 3, 7, 12, -14, -13]),
                             [4, 12, -14])

    def test_remove_duplicates(self):
        # Test remove_duplicates, which removes duplicates from a list
        self.assertListEqual(remove_duplicates([0, 1, 2, 2, 3, 4, 5, 6, 1]),
                             [0, 1, 2, 3, 4, 5, 6])

        unfiltered = [None, 'wow', 'WOW', 0, None, 1, 2, 'WOW', None]
        self.assertListEqual(remove_duplicates(unfiltered),
                             [None, 'wow', 'WOW', 0, 1, 2])
    ...
    def test_are_divisible_by_n(self):
        ...
        self.assertListEqual(are_divisible_by_n(range(100), 27),
                             [0, 27, 54, 81])

        self.assertListEqual(are_divisible_by_n(range(-100, 0), 39),
                             [-78, -39])

        self.assertListEqual(are_divisible_by_n(range(10), 2),
                             [0, 2, 4, 6, 8])
